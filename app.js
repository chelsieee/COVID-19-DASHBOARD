...
function createCard(element, i) {
    ...
    const continentButton = $(`<button type="button" class="detailButton">Show More</button>`)
...
$('#country').on("submit", (event) =>{
    ...
    console.log('print')
        const countryDetails = $.get("https://corona.lmao.ninja/v2/countries?yesterday=&sort=", (details) =>{
            country =details;
            console.log(details)
            ...
            details.forEach((element) =>{
                ...
                let countryCard =createCardforCountry(element);  
...
function createCardforCountry(element) {
    ...
    countryBody.append(newCase, totalCase, newDeath, totalDeath, newRecovered, Recovered);
